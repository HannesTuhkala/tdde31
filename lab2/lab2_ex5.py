...
statReadings = temp_lines.map(lambda p: Row(station=int(p[0])))
...
precipitation = precipitation.groupBy('year', 'month', 'station').agg(F.sum('prec').alias('prec')
precipitation = precipitation.groupBy('year', 'month').agg(F.avg('prec').alias('avgPrec')).orderBy(['year', 'month'], ascending=False).show()
